...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"  OpenAI API: {'✅ Available' if app.state.services['openai'] else '❌ Unavailable'}"
    )
    logger.info(
        f"  Pinecone: {'✅ Available' if app.state.services['pinecone'] else '⚠️  Unavailable (degraded mode)'}"
    )
    logger.info(
        f"  Redis: {'✅ Available' if app.state.services['redis'] else '⚠️  Unavailable (no caching)'}"
    )
    logger.info("=" * 60)

    if not app.state.services["pinecone"]:
        logger.info(
            "💡 To enable vector search, configure PINECONE_API_KEY and PINECONE_INDEX_NAME"
        )
...
app.include_router(
    analysis_direct_router, prefix="/api/analysis-direct", tags=["Analysis-Direct"]
)
app.include_router(
    simple_analysis_router, prefix="/api/simple-analysis", tags=["Simple-Analysis"]
)
...
async def get_service_status():
    ...
    services = getattr(
        app.state, "services", {"openai": False, "pinecone": False, "redis": False}
    )

    orchestrator_status = (
        hasattr(app.state, "orchestrator") and app.state.orchestrator is not None
    )
    ...
    return {
        "status": (
            "operational"
            if services.get("openai") and orchestrator_status
            else "degraded"
        ),
        "services": services,
...
async def simple_websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_json(
            {"type": "connection:established", "message": "Connected to CX Futurist AI"}
        )
